...
import {eatFood} from '../game/food.js';
...
export function clearSandbox() {
 ...
 S.spawnQ=0;S.bossQueue=[];S.bossLeft=0;S.victory=false;S.hazardCount=0;S.sandbox.waveActive=false;S.finalArena=null;S.qte=null;
}
...
function rebuildPlayer(){
 const old=S.players[0],p=makePlayer(num('#sbClass',0,CLASSES.length-1),0),level=num('#sbLevel',1,300);
...
function installSandbox(){
 ...
 <fieldset><legend>Frutas acumuláveis</legend><label>Fruta<select id="sbFruit">${Object.entries(FRUITS).map(([key,f])=>option(key,f.name+' · '+f.desc)).join('')}</select></label><button class="btn small" id="sbEatFruit">Consumir fruta</button><p>As frutas temporárias duram 10 segundos de simulação por coleta. As outras duram esta partida.</p></fieldset>
 ...
 $('#sbEatFruit').onclick=()=>{eatFood(S.players[0],{t:$('#sbFruit').value});toast('Fruta consumida. Bônus visível no HUD e em Status.');};
