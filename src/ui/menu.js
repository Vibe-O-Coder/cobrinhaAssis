...
export function updateOptionButtons() {
  ...
  const f = $("#fastBtn");
  if (f) f.textContent = "Velocidade: " + save.speed + "×";
  const t = $("#touchBtn");
  if (t) {
...
export function startActWave() { return save.startWave || 1; }
...
export function installMenu() {
  ...
  buildColorRow();
  buildModeRow();
  const act=$('#startAct'),wave=$('#startWave'),powers=$('#startPowers');
  act.innerHTML=Array.from({length:10},(_,i)=>'<option value="'+i+'">Ato '+(i+1)+' · '+esc(actDef(i).n)+'</option>').join('');
  wave.value=save.startWave; act.value=Math.floor((save.startWave-1)/29); powers.value=save.startPowers;
  act.onchange=()=>{save.startWave=Number(act.value)*29+1;wave.value=save.startWave;persist();};
  wave.onchange=()=>{save.startWave=Math.max(1,Math.min(290,Math.round(Number(wave.value)||1)));wave.value=save.startWave;act.value=Math.floor((save.startWave-1)/29);persist();};
  powers.onchange=()=>{save.startPowers=powers.value;persist();};
  $('#artSelect').value=save.art;
  $('#artSelect').onchange=e=>{save.art=e.target.value;persist();};
  $('#musicVolume').value=save.musicVolume;
  $('#musicVolume').oninput=e=>{save.musicVolume=Number(e.target.value);persist();};
  updateMenu();
}
