...
import {
  enemySpdMul, waveQuota, actOf,
  waveInAct, bossKind, bossCount, scaledEnemyHp, MAX_ENEMIES,
} from "../core/scaling.js";
import { S } from "../core/state.js";
import { save } from "../core/save.js";
import { rnd, ri, pick, dist } from "../core/utils.js";
import {
  EDEF, tierOf, isBoss, bossName,
  affixPoolFor,
...
/* Espécies e patentes estreiam em três etapas. A paleta segue os dez atos;
   a composição da horda segue o calendário de data/ecology.js. */
...
/* Estreias apresentam os 25 bosses; os outros encontros combinam conhecidos. */
...
export function startWave(n) {
  if(S.bossRush) n=S.bossRush.roster[S.bossRush.index].unlock;
  S.qte=null;
  S.wave = n;
  S.phase = "play";
  S.bossHazards = [];
  ...
  S.waveMod = n >= 3 && !S.bossRush ? (Math.random() < 0.3 ? MODS[0] : pick(MODS)) : MODS[0];

  for (const p of S.players) {
    if (p.dead) respawn(p);
    if (p.relicCrown) p.hp = Math.min(p.maxHp, p.hp + 2);
    if (!S.sandbox && !MODE().hardcore && save.upg.cur) p.hp = Math.min(p.maxHp, p.hp + save.upg.cur);
    p.shield = Math.max(p.shield, p.shieldBase);
  }

  for(const p of S.players) for(const ally of S.players) if(ally!==p&&!ally.dead) {
    ally.shield=Math.max(ally.shield,Math.min(3,p.coopWard||0));
    ally.hp=Math.min(ally.maxHp,ally.hp+(p.coopMend||0));
  }
  if (S.finalArena) {
    S.blocks = [];
    S.foods = [];
  ...
  if (S.bossRush || bossKind(n)) {
    ...
    S.bossQueue = S.bossRush ? [S.bossRush.roster[S.bossRush.index].id] : chooseBosses(n);
    ...
    banner(
      ...
      final
        ? "Atravesse as bordas. Fase II: sobreviva à canção em 1×; núcleo protegido. Fase III: destrua o Devorador."
        : (names.length > 1
    ...
    sfx("boss");
    q = final || S.bossRush ? 1 : S.bossLeft + Math.floor(waveQuota(n) * 0.35);
  } else {
    S.bossQueue = [];
    S.bossLeft = 0;
    q = waveQuota(n);
  ...
  if (S.waveMod.id === "swarm" && !S.finalArena && !S.bossRush) q = Math.min(MAX_ENEMIES, Math.round(q * 1.6));
  S.spawnQ = q;
  S.spawnT = 1.2;
}
