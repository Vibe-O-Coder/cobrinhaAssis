...
export function treeNode(bi, path) {
  ...
  const mag =
    Math.round(fx.m * ([0, 1, 2.5, 6][tier]) * (1 + ((h >>> 3) % 3) * 0.1) * 100) / 100;
...
/** Orçamento atômico: inclui pré-requisitos, desconta comprados e respeita conflitos. */
export function quoteBranch(bi, path = [], owned = []) {
  if(!TREE_BASES[bi] || (path.length && !treeNode(bi,path)))return {nodes:[],cost:0,blocked:0};
  const nodes=[], accepted=[...owned]; let blocked=0;
  const add = at => {
    const node=at.length?treeNode(bi,at):treeRoot(bi);
    if(accepted.includes(node.key))return true;
    if(treeBlocked(node.key,accepted)){blocked++;return false;}
    if(!canBuyNode(bi,at,accepted))return false;
    accepted.push(node.key);nodes.push(node);return true;
  };
  for(let i=0;i<path.length;i++)if(!add(path.slice(0,i)))return {nodes:[],cost:0,blocked};
  const visit=at=>{if(!add(at))return;if(at.length<MAX_DEPTH)for(let n=1;n<=3;n++)visit([...at,n]);};
  visit(path);
  return {nodes,cost:nodes.reduce((sum,n)=>sum+n.cost,0),blocked};
}

export function purchaseBranch(wallet, bi, path = []) {
  const quote=quoteBranch(bi,path,wallet.tree);
  if(!quote.nodes.length || wallet.souls<quote.cost)return false;
  wallet.souls-=quote.cost;
  for(const node of quote.nodes){wallet.tree.push(node.key);wallet.treeSpent[node.key]=node.cost;}
  return true;
}
