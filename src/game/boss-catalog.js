...
export function castCatalogBoss(e,head,newMinion,{hazard,shot,fan,summon}) {
 ...
 switch(routine){
 case 'bastion': {
  e.bossClock=4.8-phase*.25;
  e.catalogBurst={kind:move,target:{...target},clock:.9,left:3.8,n:0,phase};
  if(move===0){
    for(const side of [-1,1]){const at=point(target.x+side*360,target.y-160);circle(at.x,at.y,35,.9,.25);}
  }else if(move===1){
    for(let i=-2;i<=2;i++)if(i!==0)line(target.x+i*105,target.y-350,target.x+i*105,target.y+350,1.0+Math.abs(i)*.15,18);
  }else for(let row=-1;row<=1;row++)for(let col=-2;col<=2;col++)if((row+col)%2===0)circle(target.x+col*120,target.y+row*120,55,1.0+(row+1)*.35,.45);
  break;
 }
 case 'garden':
...
// Karkos tem baterias laterais e fileiras móveis; não reutiliza a nova de Zarath.
export function tickCatalogPattern(e,dt,shot){
 const b=e.catalogBurst;if(!b)return;b.left-=dt;b.clock-=dt;
 if(b.left<=0){e.catalogBurst=null;return;}if(b.clock>0)return;
 b.clock=.38;const n=b.n++,t=b.target;
 if(b.kind===0){
  for(const side of [-1,1]){const x=clamp(t.x+side*360,30,W-30),y=clamp(t.y-160+n*35,30,H-30);
   const a=Math.atan2(t.y-y,t.x-x)+side*Math.sin(n*.65)*.22;
   for(let i=-1;i<=1+b.phase;i++)shot(e,x,y,a+i*.13,185+n%2*45,{life:5,r:5,turn:side*.08});}
 }else if(b.kind===1){
  const gap=(n%3-1)*100;
  for(let i=-4;i<=4;i++)if(Math.abs(i*70-gap)>65){
   shot(e,clamp(t.x+i*70,20,W-20),clamp(t.y-350,20,H-20),Math.PI/2,180,{life:4,r:6});
   if(b.phase)shot(e,clamp(t.x-i*70,20,W-20),clamp(t.y+350,20,H-20),-Math.PI/2,145,{life:4,r:5});}
 }else{
  const x=clamp(t.x+(n%3-1)*190,30,W-30),y=clamp(t.y+(n%2?170:-170),30,H-30);
  for(let i=0;i<6;i++)shot(e,x,y,i*TAU/6+n*.23,140+b.phase*20,{life:3,r:4});
 }
}
