...
export function initClass(p) {
  ...
  p.ultimateTurrets = [];
  switch (p.cls) {
    case 16: p.range+=180; p.pierce++; break;
    case 6: // Berserker
      p.berserkT = 0;
      p.berserkKills = 0;
      break;
...
export function classTick(p, dt) {
  ...
  switch (p.cls) {
    case 16:
      if(p.marksmanT>0) {
        p.marksmanT-=dt; p.marksmanShot=(p.marksmanShot||0)-dt;
        if(p.marksmanShot<=0){p.marksmanShot=.12;const h=headPx(p),t=classTarget(p,h,p.range*1.5);
          if(t)shot(p,h.x,h.y,Math.atan2(t.y-h.y,t.x-h.x),760,1.5,{pierce:p.pierce+2,life:1.5,trail:true});}
      }
      return;
...
/* 🪓 BERSERKER — durante a Fúria Cega ele é imortal e NÃO OBEDECE.
   ...
   controle da cobrinha". Ao terminar, mantém todo o dano acumulado. */
...
export function classAbility(p) {
  const Hh = headPx(p);
  switch (p.cls) {
    case 16: p.marksmanT=2.4; p.marksmanShot=0; break;
    /* 🪓 Fúria Cega */
    case 6: {
