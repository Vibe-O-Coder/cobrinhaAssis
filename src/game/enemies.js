...
export function newMinion(x, y) {
  ...
  const hp = isPvp() ? d.hp * stackHpMul(stacks) : scaledEnemyHp(d, S.wave, S.players) * 0.7 * (MODE().enemyHp || 1);
  return {
    id: S.eid++, type: "mini", x, y,
    pvpXp: Math.round(2 * stackXpMul(stacks)),
    hp, mhp: hp, spd: d.spd * (isPvp() ? 1 + stacks * 0.012 : enemySpdMul(S.wave) * MODE().enemySpd), r: d.r, score: d.score,
...
function onEnemyDeath(e) {
  ...
  if(!S.sandbox) S.score += Math.round(e.score * (1 + Math.min(S.combo, 25) * 0.1));
  if (S.combo >= 3) {
...
export function updateEnemies(dt) {
  ...
  for (const e of S.enemies) {
    ...
    if (e.survival) {e.ward=1;e.dot=0;e.dotT=0;}
