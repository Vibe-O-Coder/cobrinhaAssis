...
import { FRUITS, collectFruit } from "./fruits.js";
...
export function spawnFood(normal) {
  ...
  for (let t = 0; t < 40; t++) {
    ...
    if(ty === 'n' && !isPvp() && Math.random()<.65) {
      const types=Object.keys(FRUITS); ty=types[ri(0,types.length-1)];
    }
...
export function eatFood(p, f) {
  const Hh = headPx(p);
  if(collectFruit(p,f.t)) addText(Hh.x,Hh.y-32,FRUITS[f.t].name+' · '+FRUITS[f.t].desc,FRUITS[f.t].color,1.8,12);
  if (f.t === "g") {
    if(!S.sandbox) S.score += 5;
    if (!isPvp()) if(!S.sandbox) S.runSouls += 2;
    p.grow += 2;
    heal(p, 1);
