...
import { initFruits, tickFruits } from "./fruits.js";
import { clampStats, attackInterval } from "./stats.js";
...
export function makePlayer(cls, idx) {
  ...
  const upg = S.mode === "pvp" || S.mode === 'sandbox' || MODE().hardcore ? Object.fromEntries(Object.keys(save.upg).map(k => [k, 0])) : save.upg;
  const p = {
    idx,
    hardcore: !!MODE().hardcore,
    coop: S.mode === "local" || S.mode === "online",
    cls,
    name: c.name,
    color: c.color,
    ic: c.ic,
  ...
  if (S.mode !== "pvp" && S.mode !== 'sandbox' && !p.hardcore) { applyTree(p, save.tree); finalizeTree(p); }
  // Cascavel troca vida por alcance; o resto do ajuste de classe é em initClass.
  if (cls === 10) p.range *= 1.35;
  initClass(p);
  initFruits(p);
  if(p.hardcore) p.hpCeiling=p.maxHp;
  clampStats(p);
  ...
  const startN = S.startWave>1 ? 0 : ((upg.ben || 0) + (upg.ini || 0) + (S.mode !== "pvp" && !S.sandbox && !p.hardcore && save.supplies.blessing > 0 ? 1 : 0)) * (cls === 11 ? 2 : 1);
...
export function grantPower(p, o) {
  ...
  const maxHp = p.maxHp;
  o.f(p);
  if(p.hardcore) { p.maxHp=Math.min(maxHp,p.maxHp); p.hpCeiling=p.maxHp; }
