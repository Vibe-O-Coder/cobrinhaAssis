...
export function drawMinimap(v, view = VP) {
  ...
  ctx.fillRect(x-5, y-23, w+10, h+58);
  ctx.strokeStyle = "rgba(185,168,255,.7)";
  ctx.lineWidth = 1;
  ctx.strokeRect(x-5, y-23, w+10, h+58);
  ...
  for (const p of v.players || []) {
    ...
    ctx.fillStyle = p.idx === VP.who ? "#439aff" : v.pvp ? "#ff5369" : "#fff";
  ...
  ctx.fillText("Azul: você · branco: aliado",x,y+h+11);
  ctx.fillText("Vermelho: inimigos",x,y+h+24);
  ctx.restore();
}
