...
import { drawFruit } from "./fruit-art.js";
...
export function drawEnemy(e) {
  ...
  if(save.art==='emoji'){
    ctx.font=(e.r*1.8)+'px sans-serif';ctx.textAlign='center';ctx.textBaseline='middle';
    ctx.fillText(d.final?'👹':d.boss?'👾':({runner:'🦇',shooter:'👁️',tank:'🪲',splitter:'🦠',sniper:'🦂',mini:'💀'}[d.art]||'👾'),0,0);
  } else if(!drawSprite(d,e))artFor(d, e.type)(e.r, e);
...
export function drawFood(f) {
  if(drawFruit(f.t,(f.x+.5)*CELL,(f.y+.5)*CELL))return;
  if(save.art==="emoji"){ctx.save();ctx.font="22px sans-serif";ctx.textAlign="center";ctx.textBaseline="middle";ctx.fillText(f.t==="g"?"⭐":f.t==="p"?"☠️":"🍎",(f.x+.5)*CELL,(f.y+.5)*CELL);ctx.restore();return;}
...
export function drawBossBar(v) {
  ...
  for (const b of bosses.slice(0, 3)) {
    ...
    const pct = b.survival?clamp(1-b.survival.elapsed/b.survival.duration,0,1):clamp(b.hp / b.mhp, 0, 1);
    ...
    ctx.lineWidth = 2;
    const left=b.survival?Math.max(0,Math.ceil(b.survival.duration-b.survival.elapsed)):0;
    const hpText = b.survival?'SOBREVIVA · '+Math.floor(left/60)+':'+String(left%60).padStart(2,'0')+' · 1×':Math.ceil(b.hp) + " / " + b.mhp;
