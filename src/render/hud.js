/* ================= HUD ================= */
import { fruitSummary } from "../game/fruits.js";
import {survivalTime,survivalSection,SURVIVAL_SECTIONS} from '../game/finale.js';
...
export function updateHUD() {
  ...
  for (const p of v.players) {
    ...
    hl +=
      ...
      `${p.dead ? "—" : hearts(p)}${p.shield > 0 ? " 🛡️" + p.shield : ""}${statusMarks(p)}<div class="fruit-buffs">${esc(fruitSummary(p))}</div></div>`;
  }
  $("#hudL").innerHTML = hl;
  ...
  $("#hudWave").textContent =
    ...
    "  ·  ATO " + (actOf(v.wave) + 1);
  if(S.bossRush)$('#hudWave').textContent='BOSS RUSH · '+(S.bossRush.index+1)+' / '+S.bossRush.roster.length;
  ...
  const survival=v.enemies?.find(e=>e.survival)?.survival;
  if(survival)$('#hudScore').textContent='SOBREVIVA '+survivalTime(survival)+' · '+SURVIVAL_SECTIONS[survivalSection(survival)]+' · núcleo protegido · 1×';
