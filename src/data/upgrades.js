...
const HP_UPGRADES = new Set(['coracao_ferro','coracao_valente','talisma','of_vigor','r_sangue','r_dragao','of_eco']);
UPGRADES.push(
 {id:'x_marksman',cls:16,ic:'🎯',n:'Munição de Precisão',d:'+15% dano e +30 alcance',max:5,f:p=>{p.dmgMul+=.15;p.range+=30;}},
 {id:'coop_ward',coop:true,slot:0,ic:'🛡️',n:'Vanguarda Protetora',d:'J1: +12% dano; concede 1 escudo ao aliado a cada onda.',max:3,f:p=>{p.dmgMul+=.12;p.coopWard=(p.coopWard||0)+1;}},
 {id:'coop_mend',coop:true,slot:1,ic:'💚',n:'Retaguarda Médica',d:'J2: ataque 8% mais rápido; cura 1 HP do aliado a cada onda.',max:3,f:p=>{p.cd*=.92;p.coopMend=(p.coopMend||0)+1;}},
 {id:'coop_cover',coop:true,slot:0,ic:'🔱',n:'Fogo de Cobertura',d:'J1: +1 projétil e +20 alcance.',max:3,cap:p=>p.shots>=HARD.shots,f:p=>{p.shots++;p.range+=20;}},
 {id:'coop_rescue',coop:true,slot:1,ic:'✦',n:'Resgate Ágil',d:'J2: +0,2s de invulnerabilidade após dano e +30 alcance de coleta.',max:3,f:p=>{p.iframeBonus+=.2;p.magnet=true;p.magnetR=Math.max(190,p.magnetR)+30;}}
);
RELICS.push({id:'of_resolve',ic:'✦',n:'Determinação',d:'+20% dano',overflow:true,max:Infinity,f:p=>p.dmgMul+=.2});
...
export function canOffer(o, p) {
  if (!o || !p) return false;
  if(p.hardcore && HP_UPGRADES.has(o.id)) return false;
  if(o.coop && (!p.coop || o.slot!==p.idx)) return false;
