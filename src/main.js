...
import { unlockMusic, tickMusic } from './core/music.js';
...
const ACTIONS = {
  "run-bossrush": () => gotoClass("bossrush"),
  ...
  "again": () => {
    ...
    gotoClass(
      S.lastMode === 'bossrush' ? 'bossrush' : S.lastMode === 'sandbox' ? 'sandbox' : S.lastMode === "pvp" ? "pvp" : S.lastMode === "local" ? "local" : "solo",
    );
  },
};
...
function boot() {
  const unlock=()=>{initAudio();unlockMusic();};
  document.addEventListener('pointerdown',unlock);
  document.addEventListener('keydown',unlock);
  document.addEventListener('visibilitychange',()=>tickMusic(0));
  installActions();
  installMenu();
  installIcons();
  installTreeUI();
