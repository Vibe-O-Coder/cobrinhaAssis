 ...
 */
export function RelayTransport(ticket, onRaw, onStatus, onError, onPeer, onLeave, onReady, isHost=false, onRoute=()=>{}) {
  let socket = null, alive = true, ready = false, attempt = 0, retry = null, watchdog = null, lastReply = 0;
  const queue = [];
  const direct=DirectRoute(isHost,data=>control({type:'data',data}),onRaw,onRoute);
  function control(message) { if (socket?.readyState === WebSocket.OPEN) socket.send(JSON.stringify(message)); }
  function fatal(message) { if (!alive) return; alive = false; direct.close(); clearTimeout(retry); clearInterval(heartbeat); clearTimeout(watchdog); socket?.close(); onError(message); }
  function connect() {
    if (!alive) return;
    ready = false;
    ...
    ws.addEventListener('message', event => {
      ...
      try {
        ...
        if (message.type === 'ready') {
          ...
          if (message.peer) {onPeer();direct.begin();}
          for (const data of queue.splice(0)) control({ type: 'data', data });
        } else if (message.type === 'peer') {
          direct.reset();if (message.connected) {onPeer();direct.begin();} else onLeave();
        } else if (message.type === 'data') {if(message.data?.t==='rtc')direct.accept(message.data);else onRaw(message.data);}
  ...
  return {
    ...
    send(data) {
      if (!alive) return;
      if(ready && !['bye','start','lobby'].includes(data.t) && direct.send(data))return;
      if (!ready) {
      ...
      if (data.t === 'state' && socket.bufferedAmount > 32 * 1024) return;
      control({ type: 'data', data });
    },
    close() {
      if (!alive) return;
      alive = false; direct.close(); clearTimeout(retry); clearTimeout(watchdog); clearInterval(heartbeat);
