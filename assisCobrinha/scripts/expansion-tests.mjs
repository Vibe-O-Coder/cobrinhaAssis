...
try {
  ...
  await test('Campanha sempre começa na onda 1 e Apostador recebe duas cartas sem escolher',async()=>{
    ...
    assert.equal(result.wave,1);assert.equal(result.automatic.powers,2);assert.equal(result.automatic.wave,2);assert.equal(result.automatic.phase,'play');assert.ok(result.local);assert.equal(result.gamblePowers,2);
  });
  await test('Todas as 48 especializações executam no nível X em campanha e PVP',async()=>{
    const invalid=await p.evaluate(async()=>{
      ...
      for(const mode of ['solo','pvp']) for(let cls=0;cls<16;cls++) for(let branch=0;branch<3;branch++) {
