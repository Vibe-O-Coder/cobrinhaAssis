...
try {
 ...
 await test('Árvore tem 364 nós por atributo; compra irmãos e bloqueia só especializações',async()=>{
  const r=await page.evaluate(async()=>{
   const {treeNode,treeRoot,canBuyNode,treeBlocked}=await import('/src/game/tree.js');let count=1,min=Infinity;
   const walk=p=>{if(p.length===5)return;for(let i=1;i<=3;i++){const n=treeNode(0,[...p,i]);count++;min=Math.min(min,n.cost);walk([...p,i]);}};walk([]);
   return {count,min,root:treeRoot(0).cost,sibling:canBuyNode(0,[2],['0','0-1']),conflict:treeBlocked('0-1-1-1-1-1',['2-1-1-1-1-1']),invalid:treeNode(0,[1,1,1,1,1,1])};
  });assert.equal(r.count,364);assert.ok(r.min>r.root);assert.ok(r.sibling);assert.ok(r.conflict);assert.equal(r.invalid,null);
 ...
 await test('As 16 classes usam ativas no PVP sem erros ou valores inválidos',async()=>{
