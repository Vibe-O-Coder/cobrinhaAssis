...
Relatórios reproduzíveis: `tests/artifacts/perf-before.json` e `perf-after.json`; `npm run test:perf` produz outra medição. Os testes de rede usam navegadores no mesmo computador, portanto o ping deles não representa o ping entre os dois jogadores no Brasil.
